...
;(function () {
    ...
    const variables = new Map<any, VariableDef>()
    const methods = new Map<string, string[]>()

    function mapToGLConst(value: any, methodKey: string, argNum: number) {
    ...
    const variablePrefixes = new Map<any, string>([
    ...
    function registerContextMethod(methodName: string, ...argumentTypes: string[]) {
    ...
    type CodeBlock = (VariableDef | string)[]

    const codeBlocks = [[]] as CodeBlock[]
    let activeCodeBlock = codeBlocks[0]

    function createVariable(prefix: string, arg: any): VariableDef {
    ...
    function convertArgumentToString(
        arg: any,
        methodKey: string,
        argNum: number,
        alwaysCreateVariable = false
    ): VariableDef | string {
    ...
    function getString(v: VariableDef | string): string {
        if (typeof v === "string") {
            return v
        }
        return v.name
    ...
    function isVariable(v: VariableDef | string): v is VariableDef {
        return typeof v !== "string"
    }

    function hook(proto: any, name: string) {
    ...
    function main() {
        ...
        window.dump = () => {
            ...
            for (const v of variables.values()) {
                if (v.codeToCreate.length < 1) {
                    if (ArrayBuffer.isView(v.value)) {
                        ...
                        const canvas = v.value.canvas as HTMLCanvasElement
            ...
            function writeCodeBlock(codeBlock: CodeBlock) {
                const TAB = "            "
                ...
                for (const entry of codeBlock) {
                    const code =
                        typeof entry === "string" ? entry : entry.codeToCreate.join(`\n${TAB}`)
                    if (!code) {
                        debugger
                    }
                    output += `${TAB}${code}\n`
            ...
            downloadURI(`data:text/html;charset=UTF-8,${encodeURIComponent(output)}`, "dump.html")
        }
    }
